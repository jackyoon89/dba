...
def main():
    ...
    s = subprocess.Popen([home + "/bin/crsctl", "query", "css",  "votedisk"], stdout=subprocess.PIPE)
    ...
    for out in s.stdout:
        ...
        if result <> None:
            if result.group(1).strip() <> 'ONLINE':
                mesg += "[" + result.group(3) + ":" + result.group(2) + ":" + result.group(1).strip() + "] ,"     

    
    if mesg == '':
        print "OK: All Voting Disks are Online."
