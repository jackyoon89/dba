...
# Author: jyoon
#
# 2020/06/08 - Check current number of files vs. possible maximum number of files. 
#
import sys
import os
import re
...
def main(dbname, warning, critical):
    ...
    sql = """select (select count(*) from v$dbfile) current_files, value max_files
    ...
    current, max = [files for rec in cursor for files in rec]

    percent_usage = round((float(current) / float(max) ) * 100, 2)

    if (percent_usage >= float(critical)):
        print "CRITICAL: Number of files HWM({0}% - {1}/{2})".format(percent_usage, current, max)
        sys.exit(Monitor.ERROR["CRITICAL"])
    elif (percent_usage >= float(warning) and percent_usage < float(critical)):
        print "WARNING: Number of files HWM({0}% - {1}/{2})".format(percent_usage, current, max)
        sys.exit(Monitor.ERROR["WARNING"])
    else:
        print "OK: Number of files HWM({0}% - {1}/{2})".format(percent_usage, current, max)
        sys.exit(Monitor.ERROR["OK"])
